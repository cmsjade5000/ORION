...
export default function OrbitExtras(props: {
  ...
  artifacts: Artifact[];
  feed: FeedItem[];
  arc?: { start: number; end: number } | null; // radians
  token: string;
  telegramWebApp?: any;
  onOpenFeed?: () => void;
  onOpenFiles?: () => void;
  ...
  const arc = props.arc || null;

  const angleFor = (idx: number, len: number) => {
    if (!len) return -Math.PI / 2;
    if (!arc) {
      return (idx / Math.max(1, len)) * Math.PI * 2 - Math.PI / 2;
    }
    const span = arc.end - arc.start;
    const t = (idx + 1) / (len + 1); // pad ends away from node collisions
    return arc.start + span * t;
  };

  return (
    <div className="extrasLayer">
      {items.map((it, idx) => {
        const theta = angleFor(idx, items.length);
