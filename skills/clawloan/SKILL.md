...
## Integrity Pinning and Offline Caching

To ensure security and availability, Clawloan skill loader supports manifest integrity pinning and offline caching. See `manifest.js` helper in this directory.

### Pinning Manifests

The SHA256 checksums of the remote manifests are stored in `manifest-checksums.txt` to verify authenticity:

```text
# skills/clawloan/manifest-checksums.txt
4fcd82d4ad450b2501c0f480bce23f75edf88a6c726c3d3baec1acf9443f39ad  heartbeat.md
6cbea8a243c34f653eea0e4834ebc5cc35991ccf1449041c818bc3b0b6f6e4e2  skill.json
```

### Offline Caching

Downloaded manifests are cached under `.cache/clawloan/` with a default TTL of 1 hour. When fetching fails (e.g., offline), the loader uses the cached copy if available.

### Using the Manifest Loader

Import and use the provided helper in your agent code:

```js
const { fetchManifest } = require('./manifest');

(async () => {
  const heartbeat = await fetchManifest(
    'https://clawloan.com/heartbeat.md',
    'heartbeat.md'
  );
  const skillManifest = await fetchManifest(
    'https://clawloan.com/skill.json',
    'skill.json'
  );
  // ... proceed with validated manifests
})();
```
